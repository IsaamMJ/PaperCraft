...
/// Solo teacher onboarding page (2-step wizard)
/// For solo/personal tenant teachers: grades → subjects (no sections)
class SoloTeacherOnboardingPage extends StatefulWidget {
  final String tenantId;

  const SoloTeacherOnboardingPage({
    super.key,
    required this.tenantId,
  });

  @override
  State<SoloTeacherOnboardingPage> createState() => _SoloTeacherOnboardingPageState();
}

class _SoloTeacherOnboardingPageState extends State<SoloTeacherOnboardingPage> {
          ...
          child: BlocListener<AdminSetupBloc, AdminSetupUIState>(
            listener: (context, state) {
              if (state is AdminSetupSaved) {
                // Solo teacher onboarding complete - navigate to home
  ...
  /// Build the content for the current step (only 2 steps for solo teacher)
  ...
  Widget _buildNavigationButtons(
    ...
    final isLastStep = setupState.currentStep == 2;
