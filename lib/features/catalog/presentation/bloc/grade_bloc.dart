// features/question_papers/pages/bloc/grade_bloc.dart
...
// NEW: Load assigned grades for teacher filtering
class LoadAssignedGrades extends GradeEvent {
  final String? teacherId; // null for admins
  final String? academicYear;

  const LoadAssignedGrades({
    this.teacherId,
    this.academicYear,
  });
...
class GradeBloc extends Bloc<GradeEvent, GradeState> {
  ...
  GradeBloc({required GradeRepository repository})
    ...
    on<LoadAssignedGrades>(_onLoadAssignedGrades); // NEW HANDLER
    // NEW CRUD HANDLERS
  ...
  // =============== NEW: ASSIGNED GRADES HANDLER ===============
  Future<void> _onLoadAssignedGrades(
      LoadAssignedGrades event,
      Emitter<GradeState> emit,
      ) async {
    emit(const GradeLoading(message: 'Loading assigned grades...'));

    final result = await _repository.getAssignedGrades(
